...
class OrderHistoryPageTest extends PropertProviderTest {
	...
	def "Verify OrderHistory Page Fields"(){
		...
		verifyTrue(ordersFoundLabel.text() - ~/\d+\s+/,expectedValue("text.account.orderHistory.page.totalResults").toUpperCase())
		verifyTrue(ordersFoundLabel1.text() - ~/\d+\s+/,expectedValue("text.account.orderHistory.page.totalResults").toUpperCase())
