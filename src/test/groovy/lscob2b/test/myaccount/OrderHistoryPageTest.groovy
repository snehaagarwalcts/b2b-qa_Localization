...
class OrderHistoryPageTest extends PropertProvider {
	...
	def "Verify OrderHistory Page Fields"(){
		...
		verifyTrue(orderSelectionLabels(3).text(),expectedValue("order.search.filter.MONTH.name").toUpperCase())
		verifyTrue(orderSelectionLabels(4).text(),expectedValue("order.search.filter.QUARTER.name").toUpperCase())
